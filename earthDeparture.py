...
class EarthDeparture:
    def __init__(self,
        ...
        lb = [t0[0].mjd2000] + [tf[0]]
        ub = [t0[1].mjd2000] + [tf[1]]
    ...
    def _propagate(self, x, plot = False):
        ...
        while True:
            ...
            if self.isInBurnZone(currEta) and not stopThrusting:
                ...
                rf, vf, mf = pk.propagate_taylor(ri, vi, mi, thrust, self.dt, self.__earth.mu_self, veff, -10, -10)
                # sys.exit()
        ...
        if plot:
            ...
            thrustTimes = np.array(thrustTimes)*self.__sc.thrust
            ones = np.ones(thrustTimes.shape)
            ax3.scatter(thrustTimes, ones, s = 1)
    ...
    def orbitAtAltitude(self, a, b):
        ...
        # alt = norm(ri)
        # speed = np.sqrt(self.__earth.mu_self/alt)
        return speed
    ...
    def pretty(self, x):
        ...
        mf = self.__sc.mass / np.exp(deltaV/(self.__sc.isp *pk.G0))

        dPropellant = self.__sc.mass - mf
        print("Departure:", pk.epoch(t0), "(", t0, "mjd2000 )")
        print("Time of flight:", round(T, 2), "days")
